    ...
    $.fn.autocomplete= function ()
    {
        ...
        $(this).each(function(){
            ...
            var selectItem = function(that)
            {
                ...
                $(that).parent().parent().find('#input-cta').val($(that).html());
            ...
            $(this).find('#input-cta').bind('keyup',function(event){
                var input=$(this);
                ...
                else if((code>47 && code<91)||(code>96 && code<123) || code ==8 )
                {
                    ...
                    jQuery.ajax({
                          url: '../Servicios/Buscar/' + $('#input-cta').val().trim(),
                          type: 'GET',
                          data: {id: $('#input-cta').val()},
                        ...
                        // creo un cuadro debajo del input con los resultados
                                ...
                                var result=$('<div class="cls-busqueda">');
                                ...
                                for(index in data)
                                {
                                    ...
                                    if(data.hasOwnProperty(index))
                                        {
                                            //alert(data[index]['nombre_servicio']);
                                            var a = $('<a class="cls-item">');
                                            a.html(data[index]['nombre_servicio']);
                                            a.addClass('autocomplete-jquery-item');
                                            var widthFixed=width;
                                            a.css({'width':widthFixed});
                                            a.attr('href',"/Proveedores/Servicios/"+data[index]['id']);
                    ...
                    // $.getJSON(url,{ data: {id:value }}, function(data){
                    //     // si encontro algo 
                    //     // creo un cuadro debajo del input con los resultados
                    //     input.parent().find('.autocomplete-jquery-results').remove();
                    //     var left = input.position().left;
